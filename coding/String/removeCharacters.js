function removeCharacters(str1, str2) {
    let ans = "";
    let MAX_CHARS = 26;
    let charArray1 = new Array(MAX_CHARS).fill(0);
    let charArray2 = new Array(MAX_CHARS).fill(0);
    for(let i = 0; i < str1.length; i++){
        charArray1[str1.charCodeAt(i) - 'a'.charCodeAt(0)]++;
    }
    for(let i = 0; i < str2.length; i++){
        if(charArray1[str2.charCodeAt(i) - 'a'.charCodeAt(0)] === 0) {
            ans+= str2[i];
        }
    }
